...
public:
    ...
    Animation::Pointer animation(const QPoint &point) const
    {
        ...
        else if (previousRect().contains(point))
...
protected:
    ...
    // leave event
    template<typename T>
    inline void enterEvent(const QObject *object);

    // leave event
    template<typename T>
    inline void leaveEvent(const QObject *object);

    //* mouse move event
    template<typename T>
    inline void mouseMoveEvent(const QObject *object);

    //* mouse move event
    template<typename T>
    inline void mousePressEvent(const QObject *object);
