...
public:
    ...
    */
    ...
    void setHorizontalOpacity(qreal value)
    {
        ...
        if (_horizontalData._opacity == value)
            return;
        _horizontalData._opacity = value;
        if (target() && !horizontalRect().isEmpty())
            target().data()->update(horizontalRect());
    }
    ...
    {
        value = digitize(value);
        if (_verticalData._opacity == value)
            return;
        _verticalData._opacity = value;
        if (target() && !verticalRect().isEmpty())
            target().data()->update(verticalRect());
    }
