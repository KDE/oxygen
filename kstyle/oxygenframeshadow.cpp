...
void SunkenFrameShadow::updateGeometry(QRect rect)
{
    ...
    setMargins(
        QMargins(rect.left() - parentRect.left(), rect.top() - parentRect.top(), rect.right() - parentRect.right(), rect.bottom() - parentRect.bottom()));
...
void FlatFrameShadow::updateGeometry()
{
    if (QWidget *widget = parentWidget())
        updateGeometry(widget->contentsRect());
}
