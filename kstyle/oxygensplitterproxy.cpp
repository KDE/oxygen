...
bool SplitterProxy::event(QEvent *event)
{
    ...
    case QEvent::MouseButtonRelease: {
        ...
        if (event->type() == QEvent::MouseButtonPress) {
            ...
            QMouseEvent mouseEvent2(mouseEvent->type(), _hook, mouseEvent->button(), mouseEvent->buttons(), mouseEvent->modifiers());
...
void SplitterProxy::clearSplitter(void)
{
    ...
    // send hover event
    if (_splitter) {
        QHoverEvent hoverEvent(qobject_cast<QSplitterHandle *>(_splitter.data()) ? QEvent::HoverLeave : QEvent::HoverMove,
                               _splitter.data()->mapFromGlobal(QCursor::pos()),
                               _hook);
