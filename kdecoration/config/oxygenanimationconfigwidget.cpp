    ...
    : BaseAnimationConfigWidget(parent)
{
    ...
    setupItem(layout,
              m_buttonAnimations =
                  new GenericAnimationConfigItem(this, i18n("Button mouseover transition"), i18n("Configure window buttons' mouseover highlight animation")));

    setupItem(layout,
              m_shadowAnimations =
                  new GenericAnimationConfigItem(this,
...
void AnimationConfigWidget::updateChanged(void)
{
    ...
    // track modifications
    bool modified(false);
    if (animationsEnabled()->isChecked() != m_internalSettings.data()->animationsEnabled())
        modified = true;
    else if (m_buttonAnimations->enabled() != m_internalSettings.data()->buttonAnimationsEnabled())
        modified = true;
    else if (m_buttonAnimations->duration() != m_internalSettings.data()->buttonAnimationsDuration())
        modified = true;

    else if (m_shadowAnimations->enabled() != m_internalSettings.data()->shadowAnimationsEnabled())
        modified = true;
    else if (m_shadowAnimations->duration() != m_internalSettings.data()->shadowAnimationsDuration())
        modified = true;
