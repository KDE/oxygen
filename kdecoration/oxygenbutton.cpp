...
void Button::reconfigure(void)
{
    ...
    auto d = qobject_cast<Decoration *>(decoration());
    if (d)
        m_animation->setDuration(d->internalSettings()->buttonAnimationsDuration());
}
...
void Button::paint(QPainter *painter, const QRect &repaintRegion)
{
    ...
    if (isAnimated() || isHovered() || (isToggleButton() && isChecked())) {
        ...
        if (isCloseButton())
            glow = SettingsProvider::self()->helper()->negativeTextColor(palette); // Button is close button
        else if (isHovered() && (isToggleButton() && isChecked()))
            glow = toggledHoverGlow; // Button is checked and hovered
        else if (isToggleButton() && isChecked())
            glow = toggleColor; // Button is checked but not hovered
        else
            glow = SettingsProvider::self()->helper()->hoverColor(palette); // Button is hovered but not checked
        ...
        if (isAnimated()) {
            ...
            color = glow; // If button is eigther hovered or checked, use glow color as text color
        else if ((isToggleButton() && isChecked()))
            color = toggledHoverColor; // If button is checked and hovered, use different color
    }
...
void Button::drawIcon(QPainter *painter)
{
    ...
    case KDecoration2::DecorationButtonType::Shade:
        if (!isChecked()) {
            ...
            painter->drawPolyline(QPolygonF() << QPointF(7.5, 10.5) << QPointF(10.5, 7.5) << QPointF(13.5, 10.5));
